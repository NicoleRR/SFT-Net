...
class SFT_Net(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # 最后通过线性层输出，得到64*16大小，输入到LSTM当中提取时间特征
        self.linear = nn.Linear(32 * 2 * 2, 64)
        # LSTM提取时间特征
        self.lstm1 = nn.LSTM(input_size=64, hidden_size=64, num_layers=1, batch_first=True)  # [batch, input_size, -]
        self.lstm2 = nn.LSTM(input_size=64, hidden_size=32, num_layers=1, batch_first=True)  # [batch, input_size, -]
        # 经过最后两层线性层提取分类信息
